...
from transacoes.models import Controller, ContaSuspeita, AgenciaSuspeita
...
def suspeitas(request):
    if request.method == "POST":
        ...
        if not data:
            messages.error(request, "Selecione o mês e ano (2022-04)")
            return redirect('suspeitas')

        transacoes_data = Controller.objects.filter(data_hora_transacao__startswith=data)
